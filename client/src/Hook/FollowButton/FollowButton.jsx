...
const FollowButton = ({ diffProfile }) => {
  ...
  const {
    ...
    queryKey: ["followbuts"],
    queryFn: async () => {
      return await fetch(`/api/get-following`, {
        headers: {
  ...
  const createFollowers = async () => {
    const response = await fetch(`/api/create-follow/${diffProfile}`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${user.token}`,
  ...
  const createunFollowers = async () => {
    const response = await fetch(`/api/create-unfollow/${diffProfile}`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${user.token}`,
  ...
  return (
    ...
    {diffProfile !== user._id && (
      <HeartHandshake
        ...
        className={`cursor-pointer ${folo ? "text-red-500" : "text-amber-500"}`}
      />
    )}
  </div>
